...
def load_weights_to_flatresnet(source_model, target_model):
    ...
    ss_keys = list(source_state.keys())

    # remove the module. prefix if it exists (thanks nn.DataParallel)
    if ss_keys[0].startswith('module.'):
...
def get_model(model, device):
    ...
    if model=='R32_C10':
        rnet_checkpoint = 'cv/trained_rnet/R32_C10/pk_E_164_A_0.923.t7'
        layer_config = [5, 5, 5]
        rnet = resnet.FlatResNet32(base.BasicBlock, layer_config, num_classes=10)
        agent = resnet.Policy32([1,1,1], num_blocks=15)

    elif model=='R110_C10':
        rnet_checkpoint = 'cv/trained_rnet/R110_C10/pk_E_130_A_0.932.t7'
        layer_config = [18, 18, 18]
        rnet = resnet.FlatResNet32(base.BasicBlock, layer_config, num_classes=10)
        agent = resnet.Policy32([1,1,1], num_blocks=54)

    elif model=='R32_C100':
        rnet_checkpoint = 'cv/trained_rnet/R32_C100/pk_E_164_A_0.693.t7'
        layer_config = [5, 5, 5]
        rnet = resnet.FlatResNet32(base.BasicBlock, layer_config, num_classes=100)
        agent = resnet.Policy32([1,1,1], num_blocks=15)

    elif model=='R110_C100':
        rnet_checkpoint = 'cv/trained_rnet/R110_C100/pk_E_160_A_0.723.t7'
        layer_config = [18, 18, 18]
        rnet = resnet.FlatResNet32(base.BasicBlock, layer_config, num_classes=100)
        agent = resnet.Policy32([1,1,1], num_blocks=54)

    elif model=='R101_ImgNet':
        rnet_checkpoint = 'cv/trained_rnet/R101_ImgNet/ImageNet_R101_224_76.464'
    ...
    rnet_checkpoint = torch.load(rnet_checkpoint, map_location=torch.device(device))
